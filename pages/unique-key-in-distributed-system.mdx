...
결제 시스템, 원장 관리 등과 같은 서비스에는 반드시 필요하지는 않습니다. 만약 기본키의 Range 를 늘리면서 생성시각에 따른 정렬이 가능한 기본키를 원한다면 도입을 고려하게 될 것 같습니다. <br/>
...
정산 시스템에도 굳이 메시지 대기열을 적용할 필요가 없습니다. 정산서비스의 경우 조금 느리더라도 Database 를 정확하게 이해하고 정산 작업이 조금 더 효율적으로 동작하도록 설계하는 것이 더 중요하기 때문입니다.<br/>
